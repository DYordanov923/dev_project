...
def pull_size(input_queue, output, conn, mybucket):
    try:
        while not input_queue.empty():
          ...
          #  print(image, time_st)
            ...
            try:
                ...
                duration = store_log_info(key, size, time_st, path)
...
def main_v4(profile, region, bucket_name, verbose):
   ...
   # print("queue size: " + str(_queue_size))
    for thread in range(10):
        t = threading.Thread(target=pull_size, args=[input_queue, output_queue, client, bucket_name])
        t.daemon = True
        t.start()
        threads.append(t)
